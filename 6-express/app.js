...
app.get(
  ...
  (req, res, next) => {
    ...
    res.send('Hello'); // 처리를 해 주었기 때문에 다음 미드웨어는 호출되지 않는다.
    // if (true) {
    //   return res.send('Hello');
    // }
    // res.send('Ellie'); // 한 콜백에서 response를 여러번 보낼 수 없다. if안에서 return해주기
    // 같은 경로에 여러 미드웨어가 있다면 next()를 호출하거나 처리를 해주어서 잘 동작되도록 해야한다.
  },
  (req, res, next) => {
    // console.log('first2');
    next();
...
app.use((error, req, res, next) => {
  // 항상 어플리케이션 마지막에 에러핸들러 등록하자. 중간 미들웨어에서 에러 발생해도 마지막 미드웨어가 에러를 처리해준다.
  console.error(error);
